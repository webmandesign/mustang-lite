 ...
 * The OAuth library files can be found in WebMan Amplifier plugin
 * for better updates management (in case it is required).
 * Besides, it contains a code that shouldn't be inside the theme files.
 *
		...
		class wm_twitter extends WP_Widget {
			...
			function __construct() {
				...
				//Helper variables
					...
					$atts['id']          = 'wm-twitter';
					$atts['name']        = WM_THEME_NAME . ' ' . __( 'Twitter', 'wm_domain' );
					...
					$atts = apply_filters( 'wmhook_custom_widget_atts_' . 'wm_twitter', $atts );
			...
			function form( $instance ) {
				...
				//Output
					...
					do_action( 'wmhook_wm_twitter_form', $instance );
			...
			function update( $new_instance, $old_instance ) {
				...
				//Output
					return apply_filters( 'wmhook_wm_twitter_instance', $instance, $new_instance, $old_instance );
			...
			function widget( $args, $instance ) {
					...
					//Get tweets
						if (
							...
							//Get new tweets and set new cache
								...
								//Set cache
									if ( $user ) {
										set_transient( $user_option, $user, apply_filters( 'wmhook_wm_twitter_cache_interval', 900 ) );
									}
						...
						if ( $tweets ) {
							...
							//User info
								if ( $instance['userinfo'] && ! empty( $user ) ) {
									...
									$output .= apply_filters( 'wmhook_wm_twitter_output_user', $output_user, $args, $instance );
							...
							//Tweets list
								if ( is_array( $tweets ) && ! empty( $tweets ) ) {
									...
									$output .= '<ul>' . apply_filters( 'wmhook_wm_twitter_output_tweets', $output_tweets, $args, $instance ) . '</ul>';
				...
				//Output
					echo apply_filters( 'wmhook_wm_twitter_output', $output, $args, $instance );
