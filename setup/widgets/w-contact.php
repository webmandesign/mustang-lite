	...
	class wm_contact_info extends WP_Widget {
		...
		function __construct() {
			...
			//Helper variables
				...
				$atts['name']        = WM_THEME_NAME . ' ' . __( 'Contact', 'wm_domain' );
				$atts['widget_ops']  = array(
						'classname'   => 'wm-contact-info',
						'description' => __( 'Contact information', 'wm_domain' )
					);
				$atts['control_ops'] = array();

				$atts = apply_filters( 'wmhook_custom_widget_atts_' . 'wm_contact_info', $atts );
		...
		function form( $instance ) {
			...
			//Output
				?>
				<p class="wm-desc"><?php _e( 'Displays specially styled contact information. JavaScript anti-spam protection will be applied on the email address (also, email will not be displayed when JavaScript is turned off).', 'wm_domain' ) ?></p>
				...
				<p>
					<label for="<?php echo $this->get_field_id( 'name' ); ?>"><?php _e( 'Name:', 'wm_domain' ) ?></label>
				...
				do_action( 'wmhook_wm_contact_info_form', $instance );
		...
		function update( $new_instance, $old_instance ) {
			...
			//Output
				return apply_filters( 'wmhook_wm_contact_info_instance', $instance, $new_instance, $old_instance );
			...
			//Preparing output
				...
				//Address
					if ( trim( $instance['name'] ) || trim( $instance['address'] ) ) {
						$address[10] = '<div class="address contact-info"' . wm_schema_org( 'itemprop="address"' ) . '><strong' . wm_schema_org( 'itemprop="name"' ) . '>' . $instance['name'] . '</strong><br />' . str_replace( "\r\n", '<br />', $instance['address'] ) . '</div>';
					}
				...
				//Business hours
					if ( trim( $instance['hours'] ) ) {
						...
						$address[20] = '<div class="hours contact-info"' . wm_schema_org( 'itemprop="openingHours"' ) . '>' . $instance['hours'] . '</div>';
					}

				//Phone numbers
					if ( trim( $instance['phone'] ) ) {
						$address[30] = '<div class="phone contact-info"' . wm_schema_org( 'itemprop="telephone"' ) . '>' . $instance['phone'] . '</div>';
					}
					...
					if ( trim( $instance['email'] ) ) {
						...
						if ( $matches && is_array( $matches ) ) {
							foreach ( $matches[0] as $email ) {
								$instance['email'] = str_replace( $email, '<a href="#" data-address="' . wm_nospam( $email ) . '" class="email-nospam">' . wm_nospam( $email ) . '</a>', $instance['email'] );
							}
						}

						$address[40] = '<div class="email contact-info"' . wm_schema_org( 'itemprop="email"' ) . '>' . $instance['email'] . '</div>';
					}

				//Filter the $address and prepare it for output
					$address = apply_filters( 'wmhook_wm_contact_info_address', $address, $args, $instance );
					$address = implode( '', $address );

				//Output wrapper
					if ( $address ) {
						$output .= '<div class="address-container"' . wm_schema_org( 'itemprop="sourceOrganization" itemscope itemtype="http://schema.org/LocalBusiness"' ) . '>' . apply_filters( 'wmhook_content_filters', $address ) . '</div>';
					}

			//Output
				if ( $output ) {
					echo apply_filters( 'wmhook_wm_contact_info_output', $args['before_widget'] . $output . $args['after_widget'], $args, $instance );
				}
